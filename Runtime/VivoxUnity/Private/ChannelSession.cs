...
namespace VivoxUnity.Private
{
    internal class ChannelSession : IChannelSession
    {
        ...
        public async Task ConnectAsync(bool connectAudio, bool connectText, bool switchTransmission, TimeSpan? expiration, AsyncCallback callback = null, string accessToken = null)
        {
            var tokenFetch = Client.tokenGen.GetTokenAsync(Key.Issuer, expiration ?? TimeSpan.FromSeconds(90), null, "join", null, conferenceUri: Key.ToString(), fromUserUri: Parent.Key.ToString());
            await tokenFetch;
            await Task.Factory.FromAsync(
                BeginConnect(connectAudio, connectText, switchTransmission, tokenFetch.Result, callback),
                (ar) =>
                {
                    try
                    {
