...
namespace VivoxUnity.Private
{
    internal class LoginSession : ILoginSession
    {
        ...
        private void HandleAccountLoginStateChangeEvt(vx_evt_base_t eventMessage)
        {
            ...
            {
                VivoxDebug.Instance.DebugMessage($"Vivox LoginState change encountered unexpected error: {evt.status_code}");
            }
        ...
        void WaitDeleteChannelSession(ChannelId channelId)
        {
            ...
            _channelSessions[channelId].PropertyChanged += CheckConnection;
        }

        void CheckConnection(Object sender, PropertyChangedEventArgs args)
        {
            ...
            {
                _channelSessions[session.Channel].PropertyChanged -= CheckConnection;
                DeleteChannelSession(session.Channel);
            }
        }
